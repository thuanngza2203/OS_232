...
int run(struct pcb_t * proc) {
	...
	switch (ins.opcode) {
	case CALC:
		printf("Process %d is doing CALC\n", proc->pid);
		stat = calc(proc);
		break;
	case ALLOC:
#ifdef CPU_TLB
		printf("Process %d is doing ALLOC size = %d to reg %d\n", proc->pid, ins.arg_0, ins.arg_1);
	...
	case FREE:
#ifdef CPU_TLB
		printf("Process %d is doing FREE reg %d\n", proc->pid, ins.arg_0);
	...
	case READ:
#ifdef CPU_TLB
		printf("Process %d is doing READ from reg %d + offset %d and save to %d\n", proc->pid, ins.arg_0, ins.arg_1, ins.arg_2);
	...
	case WRITE:
#ifdef CPU_TLB
		printf("Process %d is doing WRITE data %d to reg %d + offset %d\n", proc->pid, ins.arg_0, ins.arg_1, ins.arg_2);
	...
	if (stat != 0) {
		printf("There is some problem in process %d\n", proc->pid);
	}
