...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  *alloc_addr = old_sbrk;
  ...
  // printf("Allocated mem at start = %ld, end = %ld, addr = %d\n", 
  // caller->mm->symrgtbl[rgid].rg_start, caller->mm->symrgtbl[rgid].rg_end, *alloc_addr);
...
int pgalloc(struct pcb_t *proc, uint32_t size, uint32_t reg_index)
{
  ...
  printf("now fifo_pgn is %d\n", proc->mm->fifo_pgn->pgn);
...
int get_free_vmrg_area(struct pcb_t *caller, int vmaid, int size, struct vm_rg_struct *newrg)
{
  ...
  printf("  Try to print all region of process %d\n", caller->pid);
  for (struct vm_rg_struct* start = cur_vma->vm_freerg_list; start != NULL; start = start->rg_next) {
    printf("    %lu %lu %p\n", start->rg_start, start->rg_end, start->rg_next);
  }
